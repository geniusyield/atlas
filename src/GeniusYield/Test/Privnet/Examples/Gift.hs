    ...
    , testCaseSteps "plutusV2-inlinedatum" $ \info -> withSetup info setup $ \ctx -> do
        ...
        ctxRun ctx (ctxUserF ctx) $ do
          ...
          txBodyPlace <-
            buildTxBody $
              mustHaveOutput $
                mkGYTxOut addr (valueSingleton ironAC 10) (datumFromPlutusData ())
                  & gyTxOutDatumL
...
checkCollateral ::
  ...
  GYValue ->
  -- | Total collateral lovelaces.
  Integer ->
  -- | Transaction fees.
  a ->
  -- | Collateral percent (Protocol parameter).
  a ->
  Bool
