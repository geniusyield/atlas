...
import Data.Vector.Fixed qualified as V
...
generateUser UserPaths{..} =
  ...
  where
    existing = do
        skey <-
            Api.readFileTextEnvelope (Api.AsSigningKey Api.AsPaymentKey) (Api.File pathUserSKey)
                >>= \case
