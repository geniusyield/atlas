...
makeTransactionBodyAutoBalanceWrapper collaterals ss eh pp ps utxos body changeAddr outsOld = do
    ...
    collapsedBody <- first BuildTxCollapseExtraOutError $ collapseExtraOut extraOut txBodyContent (length outsOld)
...
collapseExtraOut
  ...
  -> Int
  -- ^ The amount of outputs we don't want to modify
  -> Either Api.S.TxBodyError (Api.S.TxBody Api.S.BabbageEra)
  -- ^ The updated body with the collapsed outputs
collapseExtraOut apiOut@(Api.TxOut _ outVal _ _) bodyContent@Api.TxBodyContent {txOuts} sOutsLenght
 ...
 | otherwise =
  ...
  where
    (skeletonOuts, changeOuts) = splitAt sOutsLenght txOuts
