...
finalizeGYBalancedTx :: GYBuildTxEnv -> GYBalancedTx v -> [GYTxOut v] -> Either BuildTxException GYTxBody
finalizeGYBalancedTx
    GYBuildTxEnv
...
makeTransactionBodyAutoBalanceWrapper collaterals ss eh pp ps utxos body changeAddr outsOld = do
    Api.ExecutionUnits
    ...
    collapsedBody <- first BuildTxCollapseExtraOutError $ collapseExtraOut extraOut txBodyContent outsOld
...
collapseExtraOut apiOut@(Api.TxOut outAddr outVal _ _) bodyContent@(Api.TxBodyContent {txOuts}) outsOld
 ...
 | otherwise =
    case delete apiOut cOuts of
        ...
        (sOut@(Api.TxOut sOutAddr sOutVal sOutDat sOutRefScript):_) ->
            let
  ...
  where
    outsNew = txOuts \\ map txOutToApi outsOld
    -- cOuts == change Outs == outs with the change address
    -- This is probably not neccesary but it's good to be sure we are not sending the extra lovelaces to another address
    cOuts = filter (\(Api.TxOut addr _ _ _) -> addr == outAddr) outsNew
