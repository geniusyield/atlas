...
instance Swagger.ToSchema GYMintingPolicyId where
  declareNamedSchema _ =
    pure $
      Swagger.named "GYMintingPolicyId" $
        Swagger.paramSchemaToSchema (Proxy @GYMintingPolicyId)
          & Swagger.description
          ?~ "This is the hash of a minting policy script."
            & Swagger.example
          ?~ toJSON ("ff80aaaf03a273b8f5c558168dc0e2377eea810badbae6eceefc14ef" :: Text)
...
mintingPolicyIdFromCurrencySymbol cs =
  bimap
    ...
    mintingPolicyIdFromApi
    $ Api.deserialiseFromRawBytes Api.AsPolicyId
    $ PlutusTx.fromBuiltin
    $ PlutusV1.unCurrencySymbol cs
...
mintingPolicyIdFromText policyid =
  bimap customError mintingPolicyIdFromApi
...
someScriptToReferenceApi (GYPlutusScript (GYScript v apiScript _)) =
  Api.S.ReferenceScript
    ...
    $ case v' of
      Api.PlutusScriptV1 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage v') $ Api.PlutusScript v' apiScript
      Api.PlutusScriptV2 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage v') $ Api.PlutusScript v' apiScript
      Api.PlutusScriptV3 -> Api.ScriptInAnyLang (Api.PlutusScriptLanguage v') $ Api.PlutusScript v' apiScript
 where
  v' = singPlutusVersionToApi v
someScriptToReferenceApi (GYSimpleScript s) =
  Api.S.ReferenceScript
    Api.S.BabbageEraOnwardsConway
    $ Api.ScriptInAnyLang Api.SimpleScriptLanguage
    $ Api.SimpleScript (simpleScriptToApi s)
...
someScriptFromReferenceApi
  ( Api.S.ReferenceScript
      Api.S.BabbageEraOnwardsConway
      (Api.ScriptInAnyLang Api.SimpleScriptLanguage (Api.SimpleScript s))
    ) = Just $ GYSimpleScript $ simpleScriptFromApi s
someScriptFromReferenceApi
  ( Api.S.ReferenceScript
      Api.S.BabbageEraOnwardsConway
      ( Api.ScriptInAnyLang
...
scriptFromApi script = GYScript v script apiHash
 where
  ...
  apiScript = case v' of
    Api.PlutusScriptV1 -> Api.PlutusScript v' script
   ...
   in case apiV of
        Api.PlutusScriptV1 ->
          Api.PlutusScriptWitness
            Api.PlutusScriptV1InConway
            apiV
            (Api.S.PReferenceScript (txOutRefToApi r))
        Api.PlutusScriptV2 ->
          Api.PlutusScriptWitness
            Api.PlutusScriptV2InConway
            apiV
...
anyScriptToApiScriptInEra (GYPlutusScript s@(GYScript v _ _)) = Api.ScriptInEra scriptInLanguageEra (scriptToApiScript s)
 where
  ...
  scriptToApiScript (GYScript (singPlutusVersionToApi -> v') api _) = case v' of
    Api.PlutusScriptV1 -> Api.PlutusScript v' api
    Api.PlutusScriptV2 -> Api.PlutusScript v' api
    Api.PlutusScriptV3 -> Api.PlutusScript v' api
