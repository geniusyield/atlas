...
instance Show GYStakePoolId where
    showsPrec d spId = showParen (d > 10) $
...
instance Aeson.FromJSON GYStakePoolId where
    parseJSON = Aeson.withText "GYStakePoolId" $
        either
            (fail . show)
...
instance Swagger.ToSchema GYStakePoolId where
  declareNamedSchema _ = pure $ Swagger.named "GYStakePoolId" $ mempty
                       ...
                       & Swagger.format          ?~ "hex"
                       & Swagger.description     ?~ "The hash of a public stake pool key."
                       & Swagger.example         ?~ toJSON ("c485ab20bd3f105e59f3c50a0d3fbaf615a51f70a1c6d29d00a1fd27" :: Text)
...
unsafeStakePoolIdFromText t = fromMaybe
...
instance Swagger.ToSchema GYStakePoolIdBech32 where
  declareNamedSchema _ = pure $ Swagger.named "GYStakePoolIdBech32" $ Swagger.paramSchemaToSchema (Proxy @GYStakePoolIdBech32)
                       & Swagger.description  ?~ "A stake pool id, serialised as Bech32."
                       & Swagger.example      ?~ toJSON ("pool1cjz6kg9a8ug9uk0nc59q60a67c2628ms58rd98gq587jwa2x5qt" :: GYStakePoolIdBech32)
