...
simplSpendingTxTrace Wallets {w1} = do
  ...
  withWalletBalancesCheckSimple [w1 := valueFromLovelace (-100_000_000)] . asUser w1 $ do
    -- TODO: w1 is the wallets that gets all funds for now
...
firstBetTrace dat bet ws@Wallets {w1} = do
  ...
  withWalletBalancesCheckSimple [w1 := valueNegate bet] . asUser w1 $ do
...
computeParamsAndAddRefScript ::
  ...
  GYValue ->
  Wallets ->
  m (BetRefParams, GYTxOutRef) -- Our continuation
  ...
  let
    ...
    getBalanceDiff wlBets@((getWallet, _dat, _bet) : remWalletBets) set acc =
      let wallet = getWallet ws
       ...
       in if Set.member wallet'sAddr set
            then getBalanceDiff remWalletBets set acc
            else
