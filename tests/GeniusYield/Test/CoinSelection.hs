...
tokenSalePlaceTestParams payment wallet =
  CoinSelectionTestParams
    { cstpTxExtInps = []
    , cstpTxOwnInps = []
...
runCoinSelectionTest cstrat cstParams = do
  (additionalInps, changeOuts) <-
    (`evalRand` pureStdGen)
      . runExceptT
      $ selectInputs (coinSelectionTestParamsToEnv cstParams) cstrat
...
shrinkParamsLovelace (params, el) =
  [(nParams, el) | nParams <- shrinkParams params]
