...
import GeniusYield.Types.Value (
  GYAssetClass (GYLovelace, GYToken),
  tokenNameToHex,
  valueFromList,
  valueFromLovelace,
 )
      ...
      , testCase "Some Adas and tokens" $ do
          let expected =
              ...
              res =
                utxoFromMaestro $
                  mockMaestroUtxo
                    [ maestroAssetFromLovelace 100_000_000
...
mockMaestroUtxo assets mDat mRefScript =
  Maestro.UtxoWithBytes
    { utxoWithBytesTxHash = mockTxId
    , utxoWithBytesIndex = mockTxIx
